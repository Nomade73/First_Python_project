...
### Python Port Scanner: Code Structure Overview

| **Code Component**              | **Description**                                                                                                                 |
|----------------------------------|---------------------------------------------------------------------------------------------------------------------------------|
| **1. Import Libraries**          | `import socket, logging, datetime, time, re, threading`                                                                         |
|                                  | Imports the necessary libraries for network communication, logging, time measurement, regular expressions, and multithreading.   |

| **2. Setup Logging**             | `logging.basicConfig()`                                                                                                         |
|                                  | Configures logging to store results and errors in `port_scanner.log`. This helps keep track of scan results and issues.          |

| **3. IPv4 Regex Validation**     | `ipv4_pattern = re.compile()`                                                                                                   |
|                                  | Uses regular expressions to ensure the entered IP address is valid before proceeding with the scan.                              |

| **4. Function: `scan_port()`**   | Scans a specific port on the host machine. Handles exceptions for connection errors, unreachable hosts, and invalid IPs.         |
|                                  | - **Input**: Host, Port, and Open Ports List.                                                                                   |
|                                  | - **Exception Handling**: Catches connection errors, invalid hosts, and logs them.                                               |
|                                  | - **Output**: Whether the port is open or closed.                                                                                |

| **5. Function: `port_scanner()`**| Manages scanning of a range of ports using multithreading. Creates a separate thread for each port to scan multiple ports concurrently.|
|                                  | - **Input Validation**: Checks if the port range (0-65535) is valid and that the start port is less than or equal to the end port.|
|                                  | - **Multithreading**: A new thread is created for each port in the range to perform simultaneous scanning.                       |
|                                  | - **Logging**: Logs the scan results (open or closed ports) and any errors encountered.                                          |

| **6. Main Program**              | Handles user input, validates IP addresses using regex, and triggers the port scanner function.                                  |
|                                  | - **User Input**: Takes in the host (IPv4 address) and port range (start and end ports).                                         |
|                                  | - **IPv4 Validation**: Uses regex to check if the inputted IP address is valid.                                                 |
|                                  | - **Execution**: Starts the scanning process using the `port_scanner()` function with multithreading.                            |

| **7. Time Measurement**          | `start_time = datetime.now(), end_time = datetime.now()`                                                                         |
|                                  | Measures the total time taken for the scanning process and logs the scan duration.                                               |

| **8. Output Results**            | After scanning is complete, logs and prints the open ports and the total scan duration.                                           |
|                                  | - **Logging**: Logs results (open ports) and total execution time.                                                               |
