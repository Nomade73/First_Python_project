...
| Category           | Example from Code                                           | Explanation (Ukrainian)                                                                 |
|--------------------|-------------------------------------------------------------|------------------------------------------------------------------------------------------|
| Libraries          | socket, logging, datetime, time                             | Бібліотеки, що імпортуються для використання функцій, таких як мережеві з'єднання, робота з часом і логування. |
| Classes            | None (no explicit class created)                            | У коді немає явного класу, лише функції, які реалізують поведінку.                       |
| Objects            | None (implicit object creation in functions)                | Об'єкти не створюються явно, але функції та змінні використовують об'єкти вбудованих бібліотек, таких як socket. |
| Attributes         | open_ports, target_host, start_port, end_port               | Атрибути – це змінні, що містять інформацію, наприклад, відкриті порти або параметри, введені користувачем. |
| Methods            | scan_port(), port_scanner(), logging.info(), s.settimeout(), s.connect_ex() | Методи – це функції, що виконують дії, такі як сканування портів, логування результатів, встановлення тайм-ауту. |
| Exceptions         | try-except blocks (socket.gaierror, socket.error)           | Блоки try-except використовуються для обробки помилок, наприклад, якщо хост недоступний або порт недійсний. |
| Logging            | logging.info(), logging.error()                             | Механізм логування записує повідомлення про події або помилки під час виконання програми. |
| Input Validation   | if not (0 <= start_port <= 65535)                           | Перевірка введених даних гарантує, що користувач ввів правильні значення (наприклад, IP-адресу або порти). |
| Time Measurement   | start_time = datetime.now(), end_time = datetime.now()       | Вимірювання часу використовується для обчислення тривалості сканування.                  |

**Бібліотека** — це набір готових функцій і класів, які можна використовувати в своєму коді для вирішення конкретних завдань, без необхідності писати їх самостійно. Наприклад, у Python бібліотека socket використовується для роботи з мережевими з'єднаннями.

- **Сокет** (англ. **socket**) — це програмний інтерфейс для обміну даними між двома вузлами в комп'ютерній мережі. Він дозволяє програмам на різних комп'ютерах спілкуватися між собою через інтернет або локальні мережі.
...
**Клас** — це фундаментальна концепція в об'єктно-орієнтованому програмуванні (ООП), яка дозволяє створювати об'єкти. Клас визначає структуру та поведінку об'єктів, описуючи, які властивості (атрибути) та методи (функції) ці об'єкти матимуть.

**Об'єкт** — це екземпляр класу в об'єктно-орієнтованому програмуванні (ООП). Якщо клас можна уявити як шаблон або креслення, то об'єкт — це конкретна реалізація цього шаблону з власними властивостями (атрибутами) і поведінкою (методами).

**Властивості** (атрибути) — це характеристики об'єкта, які описують його стан (наприклад, колір, розмір).
