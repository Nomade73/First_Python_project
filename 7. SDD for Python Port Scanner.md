...
|--------------------|-------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| Libraries          | socket, logging, datetime, time                             | Бібліотеки, що імпортуються для використання функцій, таких як мережеві з'єднання, робота з часом і логування.               | Libraries imported for using functions like network connections, time manipulation, and logging.                           |
...
| Methods            | scan_port(), port_scanner(), logging.info(), s.settimeout(), s.connect_ex() | Методи – це функції, що виконують дії, такі як сканування портів, логування результатів, встановлення тайм-ауту.           | Methods are functions that perform actions, such as scanning ports, logging results, or setting a timeout.                 |
...
| Input Validation   | if not (0 <= start_port <= 65535)                           | Перевірка введених даних гарантує, що користувач ввів правильні значення (наприклад, IP-адресу або порти).                  | Input validation ensures that the user has entered correct values (e.g., IP address or port numbers).                      |
